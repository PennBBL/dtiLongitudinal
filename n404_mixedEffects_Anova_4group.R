pdf("/data/joy/BBL/projects/dtilongitudinal/figures/4group_DtiComparison_Scatterplots_barplots.pdf")
...
for (i in 132:142) {
  ...
  gam_pval.fa[n, 2:(1 + dim(gam1$pTerms.table)[1])] <- gam1$pTerms.table[,3]
  gam_pval.fa[n, (2 + dim(gam1$pTerms.table)[1]):(1 
                                                          + dim(gam1$pTerms.table)[1] 
                                                          + dim(gam1$s.table)[1]) ] <- gam1$s.table[,4]
  
}

gam_pval.fa <- gam_pval.fa[!is.na(gam_pval.fa[,1]), ]
names(gam_pval.fa)[1] <- "ROI"
names(gam_pval.fa)[ 2:(1 + dim(gam1$pTerms.table)[1])] <- rownames(gam1$pTerms.table)
names(gam_pval.fa)[ (2 + dim(gam1$pTerms.table)[1]):(1 
                                                             + dim(gam1$pTerms.table)[1] 
                                                             + dim(gam1$s.table)[1]) ] <- row.names(gam1$s.table)
...
table$group1 <- c("Typically Developing","Emergents","Resilients","Persistent")
table$group1 <- factor(table$group1, levels=c("Typically Developing","Emergents","Resilients","Persistent"))
...
for (i in 132:142) {
  ...
  gam_pval.md[n, 2:(1 + dim(gam1$pTerms.table)[1])] <- gam1$pTerms.table[,3]
  gam_pval.md[n, (2 + dim(gam1$pTerms.table)[1]):(1 
                                                  + dim(gam1$pTerms.table)[1] 
                                                  + dim(gam1$s.table)[1]) ] <- gam1$s.table[,4]
...
names(gam_pval.md)[ 2:(1 + dim(gam1$pTerms.table)[1])] <- rownames(gam1$pTerms.table)
names(gam_pval.md)[ (2 + dim(gam1$pTerms.table)[1]):(1 
                                                     + dim(gam1$pTerms.table)[1] 
                                                     + dim(gam1$s.table)[1]) ] <- row.names(gam1$s.table)
...
for (i in 132:142) {
  ...
  gam_pval.rd[n, 2:(1 + dim(gam1$pTerms.table)[1])] <- gam1$pTerms.table[,3]
  gam_pval.rd[n, (2 + dim(gam1$pTerms.table)[1]):(1 
                                                  + dim(gam1$pTerms.table)[1] 
                                                  + dim(gam1$s.table)[1]) ] <- gam1$s.table[,4]
...
names(gam_pval.rd)[ 2:(1 + dim(gam1$pTerms.table)[1])] <- rownames(gam1$pTerms.table)
names(gam_pval.rd)[ (2 + dim(gam1$pTerms.table)[1]):(1 
                                                     + dim(gam1$pTerms.table)[1] 
                                                     + dim(gam1$s.table)[1]) ] <- row.names(gam1$s.table)
...
for (i in 132:142) {
  ...
  gam_pval.ad[n, 2:(1 + dim(gam1$pTerms.table)[1])] <- gam1$pTerms.table[,3]
  gam_pval.ad[n, (2 + dim(gam1$pTerms.table)[1]):(1 
                                                  + dim(gam1$pTerms.table)[1] 
                                                  + dim(gam1$s.table)[1]) ] <- gam1$s.table[,4]
...
names(gam_pval.ad)[ 2:(1 + dim(gam1$pTerms.table)[1])] <- rownames(gam1$pTerms.table)
names(gam_pval.ad)[ (2 + dim(gam1$pTerms.table)[1]):(1 
                                                     + dim(gam1$pTerms.table)[1] 
                                                     + dim(gam1$s.table)[1]) ] <- row.names(gam1$s.table)
...
write.csv(gam_corr.fa, "/data/joy/BBL/projects/dtilongitudinal/output/dtiLongitudinal_anova4group_n404_fa.csv", row.names=F)
write.csv(gam_corr.md,"/data/joy/BBL/projects/dtilongitudinal/output/dtiLongitudinal_anova4group_n404_md.csv", row.names=F)
write.csv(gam_corr.rd, "/data/joy/BBL/projects/dtilongitudinal/output/dtiLongitudinal_anova4group_n404_rd.csv", row.names=F)
write.csv(gam_corr.ad, "/data/joy/BBL/projects/dtilongitudinal/output/dtiLongitudinal_anova4group_n404_ad.csv", row.names=F)
